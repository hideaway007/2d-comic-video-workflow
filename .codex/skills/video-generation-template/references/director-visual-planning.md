...
竖屏视频视觉导演方案。你的任务不是重写内容，而是控制全片视觉连续性：
场景组织、空间关系、镜头语言、运镜节奏、景别比例、灯光方案、色彩递进、
entity 动线、asset 位置、信息揭示顺序，以及不同 image workers 之间的一致性。

## 输入

- `01_script/narration.md`
- `02_prompts/timeline_beats.json`
- `00_brief` 前期策划包、reference selection plan 和用户风格约束
...
```json
{
  ...
  "beat_directives": [
    {
      ...
      "lighting": "本 beat 灯光",
      "composition": "构图与空间调度",
      "entity_blocking": "entity 站位与动作",
      "asset_blocking": "关键 asset 位置",
      "setting_focus": "场景信息",
      "avoid": []
    }
  ],
  ...
  "worker_prompt_rules": {
    "must_include": [],
    "must_not_include": [],
    "reference_policy": "如何传递 entity、setting、总平面和动线参考",
...
- 镜头设计必须服务节奏：开场多建立空间，中段增加过肩 / 中景压迫，高潮才允许
  少量特写和插入镜头。
- 默认限制大头特写；除非 beat 是关键反应、关键 asset、信息揭示或声音来源，
  否则优先使用远景、中远景、过肩、鸟瞰、走廊纵深、院落总览。
- 镜头比例必须可统计：空间镜头（`wide`、`medium_wide`、`medium`、
  `over_shoulder`、`bird_eye`）默认不少于 55%；`close_up` 默认不超过 15%；
  `insert` 只用于关键 asset、声音来源或信息揭示。
- 每个主要 setting 必须有空间连续性：入口、主体区域、关键背景、前景 asset 和
  行动路径的相对位置不能随机漂移。
- 每段都要明确 entity 动线：谁从哪里来、停在哪里、看向哪里、下一个动作通向哪里。
- 灯光必须成体系：自然天光、室内灯、屏幕光、街灯、产品高光或其他主光源不能
  每张随机变化。
- 信息要递进揭示：先建立空间和状态，再给出关键 asset 或界面线索，最后进入反应
  或结果；不要过早把最大视觉奇观塞进前段。
...
- 把 `shot_scale`、`camera_angle`、`camera_motion`、`lighting`、`composition`、
  `entity_blocking`、`asset_blocking` 写入图片 prompt。
...
- 把 `space_bible` 中的总平面、setting 平面、entity 动线作为 reference pack 传给 worker。
