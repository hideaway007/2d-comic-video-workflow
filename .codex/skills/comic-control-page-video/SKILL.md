...
description: 当用户已有通过 comic-upstream-planner 验证的 00_brief 前期策划包，或明确提供完整可用口播，并希望在 2d漫画 项目中生成 9:16 单页竖图、豆包语音、字幕对齐和 2.5D 漫画视频时使用；若只有标题、小说、剧本粗 brief，先用 comic-upstream-planner。
---

# 9:16 单页漫画视频工作流

把已通过前期策划包的题材，或用户明确提供的完整可用口播，做成可审核、可交接、
可验证的 9:16 单页竖图漫画视频流水线。产出物包括口播 / 故事包、
beat-first 时间轴、专业导演视觉统筹、参考图包、9:16 单页剧情图、豆包语音、
Remotion 2.5D 视频和烧录字幕版视频。

默认只在 Phase 1A 口播稿后保留人工审核停顿。用户通过口播后，后续图片、音频、
视频和字幕阶段用机器 gate、QA、`ffprobe` 和项目验证命令继续推进；除非用户明确
要求再次人工审核，不得把图片或后期阶段卡在人工确认。

本 Skill 是当前项目的视频生产入口。前期策划由 `$comic-upstream-planner` 负责；
如果没有通过验证的 `00_brief/upstream_planning_audit.json`，先切到
`$comic-upstream-planner`。本 skill 只保留口播审核、阶段 gate、交接契约和验证命令；
题材写作与视觉细则必须进入对应 topic pack，不要把历史与中式克苏鲁规范混写在主页面。
...
- 用户已经给出通过 `$comic-upstream-planner` 验证的 `00_brief` 前期策划包。
...
Preflight  前期策划包检查
   ...
   0.2  缺失或失败时，先使用 $comic-upstream-planner
   ▼
Phase 1A  口播稿与审核包
   1.1  读取 00_brief 前期策划包，路由到 history 或 chinese-cthulhu topic pack
...
Phase 0 不再由本 skill 负责。除非用户已经提供完整可用口播并明确要求跳过，
默认必须先用 `$comic-upstream-planner` 生成并验证 `00_brief` 前期策划包。

本 skill 只消费策划包，不维护 Phase 0 schema，也不在视频阶段重新规划世界观、
事件、人物、场景或道具。Phase 1A 必须从 `00_brief` 的策划包写观众可听的
口播稿，不得绕过策划包重新编一版无关故事。
...
| 阶段 | 必读 | 按需查 |
|---|---|---|
| Preflight 前期策划包检查 | `00_brief/upstream_planning_audit.json`；缺失或失败时使用 `$comic-upstream-planner` | 用户明确要求跳过前期策划的原因 |
| Phase 1A 口播稿与审核包 | `00_brief/story_strategy.json` + `00_brief/event_scene_map.json` + 对应 topic pack | 用户原始 brief / 剧本 / 题材约束 |
...
1. 读取并遵守 `00_brief` 前期策划包，不得绕过已锁定的事件、场景、人物和道具。
2. 按题材路由读取对应 topic pack。
3. 直接生成完整中文稿件。历史模式交付江湖野史口播稿；中式克苏鲁模式交付
   原创怪谈 / 可转口播稿。
4. 写 `story_package.json`、口播正文和审核包。
5. 历史模式必须写 `historical_fact_base.md`。
6. 中式克苏鲁模式必须写 `fiction_boundary.md`，且不写真实历史事实底座。
7. 生成后先按 topic pack 的自检规则修一遍，再交给用户审核。
...
- `00_brief/upstream_planning_audit.json.passed === true`，或在
  `01_script/story_package.json` 中记录用户明确要求跳过 Phase 0 的原因。
...
- contact sheet 已生成，作为 QA artifact 和最终交付索引；默认不因此停下等人工审核。
...
只有用户明确要求“先看图 / 等我确认图片”时，才把
`03_images/image_review_status.json.status` 设为 `pending_human_review` 并停住。
默认路径下，机器 gate 通过即可进入音频和视频阶段；机器 gate 未通过仍是 blocker。
...
最终交付前不得只依赖估算时间轴。真实音频生成后，必须把三个时间面全部收齐：

- `05_video/audio/voice_timeline.json`：`segments` 和 `cues` 都必须来自真实
  `master.mp3` 时长；最终字幕优先使用 forced alignment / ASR word timings 生成的
  `cues[*].startMs/endMs`，不得按文本长度平均摊时间。
- `05_video/motion_comic_preview.mp4`：如果使用 `compose:preview`，ffconcat 必须为
  每个 shot 写入来自 `voice_timeline` 的 `duration`，合成总时长必须等于
  `voice_timeline.totalMs`。
- `05_video/subtitles/subtitle-layer-segmented-white.ffconcat`：必须把 cue 之间的无字
  停顿和尾部空白写成透明 `subtitle_blank.png` duration，不能只连续拼接有字帧；
  concat 总时长必须等于 `voice_timeline.totalMs`。
...
- 如果已有外部 forced alignment JSON，必须运行 `npm run align-audio`，用真实
  对齐结果替换估算边界。
- 如果没有外部 alignment，但需要最终字幕与口播严格配对，优先运行
  `npm run align-audio -- project_output/control-page-runs/<run> -- --transcribe mlx-whisper`
  或等价真实转写流程，生成 `forced_alignment.json`、`forced_alignment_report.json`
  和带真实 `cues` 的 `voice_timeline.json`。
...
深色描边和阴影。字幕字号必须固定，默认 `66px`，不得因为短句 / 长句每次动态
缩放字号；字幕位置锚点在画面高度 `0.75`，也就是从下往上约 `1/4` 处，
`1080x1920` 默认 `anchor_y_px=1440`、`bottom=480`。

字幕和口播配对的权威不是字幕文本顺序，而是 `voice_timeline.json.cues[*]` 的真实
`startMs/endMs`。`burn-subtitles` 必须：

- 按 cue 的真实开始 / 结束时间生成字幕事件。
- 对 cue 之间的 gap 写入透明空帧，保留语音停顿。
- 对片尾剩余时长写入透明空帧，保证字幕图层总长等于音频时间轴。
- 生成 `subtitle-events-segmented-white.json`、
  `subtitles-segmented-white.zh.ass` 和
  `subtitle-layer-segmented-white.ffconcat` 作为可审计产物。

如果用户反馈“声音和字幕不配对”，优先检查 `voice_timeline.cues` 与
`subtitle-layer-segmented-white.ffconcat` 的总时长和 gap，而不是只调字幕样式。
...
- 如果本轮使用真实豆包语音，必须以 `voice_timeline.json` 的真实时长回写 motion
  plan，不得继续使用估算时长。
- 字幕版必须验证 `subtitle-layer-segmented-white.ffconcat` 的 duration 总和等于
  `voice_timeline.totalMs`，且包含必要的透明空帧 gap；不能交付压缩掉停顿的字幕层。
...
- `00_brief/story_strategy.json`、`event_scene_map.json`、`character_registry.json`、
  `environment_registry.json`、`prop_registry.json`、`reference_selection_plan.json`
...
- `voice_timeline.json.cues[*].startMs/endMs` 来自真实音频对齐；如使用 forced
  alignment，`forced_alignment_report.json` 已写入并无未解释的大面积 fallback。
- 字幕事件和字幕 ffconcat 已生成，`subtitle-layer-segmented-white.ffconcat` duration
  总和等于 `voice_timeline.json.totalMs`，并保留无字 gap。
...
| 文件 | 何时读 | 内容 |
|---|---|---|
| `$comic-upstream-planner` | Preflight 缺少通过验证的 `00_brief` 时使用 | 生成 story_strategy、event_scene_map、registry、reference_selection_plan 和 adaptation_plan |
...
| `scripts/build-forced-aligned-voice-timeline.mjs` | Phase 3 可选 | 一次性完整口播音频 + forced alignment / `--transcribe mlx-whisper` 生成标准 `voice_timeline.json` 和真实字幕 cue |
